...
;     Aulas/Disciplinas), eliminando a necessidade de digitar letras a cada curso.
;   - Interface 100% Padronizada com o Painel de Controle:
;     Todas as caixas de mensagem, formulários de seleção em lote, confirmações,
;     telas de validação com rolagem e relatório final agora utilizam a biblioteca
;     UI_Moderna.ahk (Segoe UI, tons #F7F9FC e #003366).
;   - Geração Dupla:
;     Salva os PDFs na pasta compartilhada do OneDrive/SharePoint e mantém
...
for numCursoAtual, cursoSelecionado in selecionados {
    ...
    try {
        ...
        if (coordLista.Length = 1) {
            ...
            ; Tenta automaticamente TODOS os coordenadores possíveis
...
for cfg in cursoConfigs {
    for abaNome in cfg["abas"] {
        ...
        ; ─── Detectar colunas individualmente em cada aba ───────────────
...
DetectarColunasPorAba(wsAba) {
    ...
    ; Fallback: se não encontrar "Docente" nem "Aula" na linha 3,
    ; usa as colunas padrão mais comuns (E para docente, D para aula).
    if (colNome = "")
        colNome := "E"
    if (colCurso = "")
        colCurso := "D"
...
SanitizarNomeArquivo(nome) {
    ...
    limpo := RegExReplace(limpo, "[\x00-\x1F]", "")     ; caracteres de controle invisíveis
    limpo := Trim(limpo, " `t`r`n.")                     ; Windows não aceita pasta/arquivo terminado em espaço ou ponto
    if (limpo = "")
        limpo := "Sem_Nome"
    return limpo
}
...
ResolverCelulaMesclada(cel) {
    ; Tenta redirecionar para a célula âncora (primeira da área mesclada)
    ; Retorna a própria célula se não for mesclada ou em caso de erro
    try {
        if cel.MergeCells
...
;   Tenta: .Text (formatado) → .Value2 (serial Excel) → .Value (fallback)
; =========================================================
...
LerRegistro(wsCurso, linha, colNome, colCurso) {
    ...
    ; Detecta se alguma célula da linha era parte de uma mescla
    mesclado := false
    ...
    ; Lê a data usando o método robusto (Text + Value2 + fallback)
    ...
    ; Remove parênteses e o que estiver dentro (ex.: "Fulano de Tal (Einstein)")
    nome := RegExReplace(nome, "\s*\([^)]*\)", "")
    ; Remove título no início do nome (ex.: "Dr. Fulano de Tal", "Profa Beltrana")
...
GerarCertificadosCurso(cfg, ppt, pastaSaidaBase, linhasParaPular) {
    ...
    for abaNome in cfg["abas"] {
        ...
        ; ─── Garante criação de pastas tanto na Nuvem quanto no Backup Local ───
...
GerarCertificado(registro, ppt, pptModelo, pastaNuvemMes, pastaLocalMes, pastaDocente, nomePos) {
    ...
    try {
        ...
        ; 1) Salva o PDF na pasta principal (Nuvem SharePoint/OneDrive se disponível, senão Local)
        caminhoPrincipal := ""
        ...
        ; 2) Copia de Backup Local por Mês (C:\Certificados\<Curso>\<MM.AA>\)
        ...
        ; 3) Copia de Backup Local por Docente (C:\Certificados\<Curso>\Docentes\<Nome>\)

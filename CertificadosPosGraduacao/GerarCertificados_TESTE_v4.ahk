...
;
;   ── MODO TESTE (GerarCertificados_TESTE_v4) ─────────────────────────────
;   Mesma lógica da v24, para testes e demonstrações, sem mandar e-mail para
;   docente real e sem misturar arquivos de teste com declarações de verdade:
;   - Passo 0 pergunta um e-mail de teste; no manifesto.txt o "email" de
;     TODOS os docentes é trocado por ele (TESTE_v2/v3).
;   - v4: todo PDF de teste começa com "TESTE - " no nome e é salvo SÓ em
;       C:\Certificados_TESTE\<Curso>\<Mês>\                (cópia local)
;       Declarações\Para Enviar\TESTE - <Curso>\<Mês>\      (para o fluxo)
;     Nada vai para o arquivo oficial (Declarações\<Semestre>\...), nem para
;     C:\Certificados\<Curso>\ ou para as pastas por docente. Ao final do
;     teste dá pra apagar a pasta C:\Certificados_TESTE inteira e as pastas
;     "TESTE - ..." dentro de "Para Enviar" sem risco de apagar declaração
;     real. A pasta "TESTE - <Curso>" mantém os 2 níveis que o fluxo exige.
;   Regra: toda mudança na versão oficial também é feita aqui; o salvamento
;   separado acima é exclusivo do teste (pedido do usuário em 25/09/2026).
;   O nome do arquivo NÃO segue o padrão "GerarCertificados_v<N>.ahk" de
;   propósito — assim o Painel e o GerarEEnviar nunca escolhem esta versão
;   sozinhos; ela só roda quando aberta diretamente.
; ════════════════════════════════════════════════════════════════════════
...
global PASTA_TESTE       := "C:\Certificados_TESTE\"   ; MODO TESTE (v4): única pasta local dos PDFs de teste
global PREFIXO_TESTE     := "TESTE - "                 ; MODO TESTE (v4): início do nome de todo PDF e da pasta em "Para Enviar"
...
; =========================================================
; PASSO 0 — MODO TESTE: E-MAIL QUE RECEBE TODOS OS ENVIOS
; =========================================================

Loop {
    inputEmailTeste := PedirTexto(
        "Passo 0 — Modo Teste",
        "MODO TESTE — E-mail de Teste",
        "Esta é a versão de TESTE do gerador.`n`n"
        "Os certificados serão gerados normalmente, mas no manifesto.txt o e-mail`n"
        "de TODOS os docentes será trocado pelo e-mail que você digitar abaixo.`n"
        "Assim, o Power Automate manda tudo só para esse endereço.",
        "",
        "Ex.: seu.nome@einstein.br"
    )
    if (inputEmailTeste["Result"] != "OK")
        ExitApp

    EMAIL_TESTE := Trim(inputEmailTeste["Value"])
    if !EmailEhValido(EMAIL_TESTE) {
        ExibirMensagem(
            "Modo Teste",
            "E-mail Inválido",
            "O e-mail digitado não parece válido:`n`n" EMAIL_TESTE "`n`nDigite novamente.",
            "erro", "Tentar de Novo"
        )
        continue
    }
    ...
    if Confirmar(
        ...
        "Todos os e-mails do manifesto irão para:`n`n📧 " EMAIL_TESTE "`n`nEstá correto?",
        "✅ Sim, Continuar",
        "✏️ Corrigir"
    )
        break
}
RegistrarLog("MODO TESTE — todos os e-mails do manifesto irão para: " EMAIL_TESTE)
...
resumoFinal := "⚠️ MODO TESTE — e-mails do manifesto: " EMAIL_TESTE "`n"
resumoFinal .= "PDFs de teste (nome começa com '" PREFIXO_TESTE "') em:`n"
resumoFinal .= "   " PASTA_TESTE "`n"
resumoFinal .= "   Para Enviar\" PREFIXO_TESTE "<Curso>\<Mês>`n"
resumoFinal .= "Depois do teste, pode apagar essas pastas.`n`n"
...
ExibirRelatorioFinalUI(
    ...
    PASTA_TESTE,   ; MODO TESTE (v4): o botão de abrir pasta leva pra pasta de teste
    700
)
...
GerarCertificadosCurso(cfg, pastaSaidaBase) {
    ...
    for dadosAba in cfg["dadosAbas"] {
        ...
        ; MODO TESTE (v4): nada vai pro arquivo oficial (Declarações\<Semestre>\...),
        ; nem pro C:\Certificados\<Curso>\, nem pras pastas por docente. Só:
        ;   C:\Certificados_TESTE\<Curso>\<Mês>\  e  Para Enviar\TESTE - <Curso>\<Mês>\
        pastaNuvemMes := ""
        pastaLocalMes := PASTA_TESTE nomeCursoComUnidade "\" mesPasta "\"
        CriarPastaSegura(pastaLocalMes)
        pastaDocentesRaiz := ""
        pastaParaEnviarMes := GarantirPastaParaEnviar(PREFIXO_TESTE nomeCursoComUnidade, mesPasta)
        RegistrarLog("MODO TESTE — aba " abaNome " → local: " pastaLocalMes " | Para Enviar: " pastaParaEnviarMes)
        ...
        for reg in dadosAba["registros"] {
            ...
            pastaDocente := ""   ; MODO TESTE (v4): sem cópia na pasta por docente
...
GerarManifestoDoMes(pastaParaEnviarMes, manifestoDocentes, ordemManifesto, cfg, abaNome) {
    ...
    ; MODO TESTE: troca o e-mail de todos os docentes pelo e-mail de teste.
    ; Cópia do Map pra não alterar o registro original do docente.
    listaDocentesTeste := []
    for doc in listaDocentesValidos {
        docTeste := doc.Clone()
        docTeste["email"] := EMAIL_TESTE
        listaDocentesTeste.Push(docTeste)
    }
    listaDocentesValidos := listaDocentesTeste
...
GerarCertificado(registro, pptModelo, pastaNuvemMes, pastaLocalMes, pastaDocente, pastaParaEnviar, nomePos, criados) {
    ...
    try {
        ...
        nomeBase := PREFIXO_TESTE nomeLimpo " (" dataLimpa ")"   ; MODO TESTE (v4): "TESTE - Nome (data).pdf"
...
IniciarLog() {
    ...
    if !DirExist(pastaLogs)
        pastaLogs := A_Temp "\"
    LOG_ARQUIVO := pastaLogs "geracao_TESTE_" FormatTime(A_Now, "yyyy-MM-dd_HH-mm-ss") ".txt"   ; MODO TESTE (v4)

...
;     listagem de abas.
;   - v19: duas correções no salvamento dos PDFs —
;     (1) GarantirPastasSaida (em UI_Moderna.ahk) agora procura uma pasta já
...
;   - v20: os parâmetros extras que a v18 passou pro Workbooks.Open (pra
;     suprimir os diálogos de "Atualizar Links"/"Somente Leitura") fizeram
;     o Excel de alguns computadores rejeitar a abertura da planilha mestre
;     com erro 0x800A03EC ("Application-defined or object-defined error").
;     AbrirPastaDeTrabalho agora tenta essa forma primeiro e, se o Excel
;     recusar, tenta de novo com parâmetros mais simples antes de desistir.
;     excel.DisplayAlerts e excel.AskToUpdateLinks continuam desligados
;     (isso não depende dos parâmetros do Open e não deve causar esse erro).
; ════════════════════════════════════════════════════════════════════════
...
AbrirPastaDeTrabalho(excel, caminho) {
    ...
    ;
    ; Tenta primeiro com os parâmetros que suprimem os diálogos de "Atualizar
    ; Links"/"Somente Leitura". Em alguns computadores/versões do Excel esses
    ; parâmetros extras fazem o próprio Open() falhar com erro 1004/0x800A03EC
    ; ("Application-defined or object-defined error") — nesse caso, tenta de
    ; novo com uma chamada mais simples antes de desistir.
    try {
        return excel.Workbooks.Open(caminho, 0, false, , , , true)
    } catch {
    }
    try {
        return excel.Workbooks.Open(caminho, 0)
    } catch {
    }
    return excel.Workbooks.Open(caminho)
}

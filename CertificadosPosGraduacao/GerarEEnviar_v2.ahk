...
; ════════════════════════════════════════════════════════════════════════
;   GERAR E ENVIAR — v2 — LANÇADOR (roda os dois programas em sequência)
;   v2: em vez de um nome de arquivo fixo, busca automaticamente na pasta
;   o arquivo com a versão mais recente de cada módulo (mesmo mecanismo
;   de detecção do Painel de Controle) — assim este lançador continua
;   funcionando sozinho quando GerarCertificados/EnviarEmails ganham uma
;   versão nova, sem precisar editar este arquivo.
; ════════════════════════════════════════════════════════════════════════
;
...
PREFIXO_GERAR  := "GerarCertificados_v"
PREFIXO_ENVIAR := "EnviarEmails_v"

pastaScripts := A_ScriptDir

caminhoGerar := LocalizarScriptMaisRecente(pastaScripts, PREFIXO_GERAR)
if (caminhoGerar = "") {
    MsgBox(
        "Não encontrei nenhum arquivo com o padrão `"" PREFIXO_GERAR "<numero>.ahk`" nesta pasta:`n`n" pastaScripts "`n`n"
...
caminhoEnviar := LocalizarScriptMaisRecente(pastaScripts, PREFIXO_ENVIAR)
if (caminhoEnviar = "") {
    MsgBox(
        "Não encontrei nenhum arquivo com o padrão `"" PREFIXO_ENVIAR "<numero>.ahk`" nesta pasta:`n`n" pastaScripts "`n`n"
        "Na próxima tela, localize esse arquivo manualmente.",
...
; ════════════════════════════════════════════════════════════════════════
;   Busca o arquivo mais recente na pasta pelo padrão "Prefixo_v<numero>.ahk"
; ════════════════════════════════════════════════════════════════════════

LocalizarScriptMaisRecente(pasta, prefixo) {
    melhorCaminho := ""
    melhorVersao  := -1
